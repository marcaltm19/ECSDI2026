...
def comunicacion():
    ...
    if not message:
        return ('<html><head><title>AgenteDevolucion</title></head>'
                '<body style="font-family:sans-serif;padding:32px"><h2>AgenteDevolucion</h2>'
    ...
    try:
        ...
        logger.warning(f'[Devolucion] /comm parse error: {e}')
        gr = build_message(Graph(), ACL['not-understood'], sender=DevolucionAgent.uri, msgcnt=mss_cnt)
        mss_cnt += 1
